...
SELECT
    EXTRACT(YEAR FROM p.paymentDate) AS SalesYear,
    COUNT(p.customerNumber) AS ProductsSold,
    SUM(p.amount) AS TotalValue,
    MIN(p.amount) AS MinPayAmount,
    (SUM(p.amount) / (SELECT SUM(amount) FROM payments p WHERE EXTRACT(YEAR FROM paymentDate) = EXTRACT(YEAR FROM paymentDate))) * 100 AS PercentageValue
FROM payments
GROUP BY SalesYear, yt.YearlyTotal
ORDER BY SalesYear;
